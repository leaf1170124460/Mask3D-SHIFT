...
def format_seg_results(seg_results):
    seg_results_list = []
    # each batch
    for bi in range(len(seg_results)):
        new_seg_results = []
        # each camera
        for vi in range(len(seg_results[bi])):
            segments_info = []
            seg = seg_results[bi][vi]
            ...
            for idx in inds:
                ...
                if not idx == bg_id:
                    ...
                    area = (seg == idx).sum()
                    mask = (seg == idx)
                    rle = mask_util.encode(np.array(mask[:, :, np.newaxis], order='F', dtype=np.uint8))[0]
                    ...
                    segments_info.append({
                        ...
                        'area': area,
                        'rle': rle})
            new_seg_results.append(segments_info)

        seg_results_list.append(new_seg_results)
        return seg_results_list
