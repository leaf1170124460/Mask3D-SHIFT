    ...
    def show_results(self, data, result, out_dir, show=False, score_thr=None):
        ...
        for batchindex in range(len(img_metas)):
            if 'gt_bboxes_3d' in data:
                ...
                with_gt = False
            for vi in range(len(img_metas[batchindex]['filename'])):
                img_path = img_metas[batchindex]['filename'][vi]
                ori_img = mmcv.imread(img_path)
                if 'data_aug' in data:
                    ori_img = Image.fromarray(np.uint8(ori_img))
                    resize, resize_dims, crop, flip, rotate = data['data_aug'][batchindex]
                    resize = resize.item()
                    resize_dims = [v.item() for v in resize_dims]
                    crop = [v.item() for v in crop]
                    flip = flip.item()
                    rotate = rotate.item()
                    ori_img, _ = self.img_transform(ori_img, resize, resize_dims, crop, flip, rotate)
                ...
                vis_image = draw_lidar_bbox3d_on_img(bboxes, ori_img, lidar2img, img_metas=None)

                if with_gt:
                    gt_img = draw_lidar_bbox3d_on_img(gt_bboxes, gt_img, lidar2img, img_metas=None)

                    vis_image = np.concatenate([gt_img, vis_image])
                    # vis_image=cv2.resize(vis_image,(1000,1000))
                cv2.imshow('visualize', vis_image)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
                # cv2.imwrite(osp.join('visualize/detr3d/',img_metas[batchindex]['sample_idx']+'-'+img_path.split('/')[-1]),vis_image)
